    ...
    public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk,
        ...
        commandBuffer.run(deferredStore -> {
            try {
                ...
                TransformComponent transformComponent = deferredStore.getComponent(player.getReference(), TransformComponent.getComponentType());
                ...
                var spatialResourceType = BlockModule.get().getItemContainerSpatialResourceType();
                var spatial = chunkStore.getStore().getResource(spatialResourceType);

                List<Ref<ChunkStore>> objectList = SpatialResource.getThreadLocalReferenceList();
                spatial.getSpatialStructure().collect(playerPos, 5.0, objectList);

                if (!objectList.isEmpty()) {
                    Ref<ChunkStore> ref = objectList.getFirst();
                    if (ref.isValid()) {
                ...
                if (lootChestConfig.isParticlesAppear()) {
                    List<Ref<ChunkStore>> objectList1 = SpatialResource.getThreadLocalReferenceList();
